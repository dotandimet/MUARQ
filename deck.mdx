...
--- 
## Roles
 - Dynamic multiple inheritance: compose class + role(s) to create custom sub-class
 - Error if two different roles add a new method with the same name
 - `Class::Method::Modifiers` lets us wrap a method so that multiple overrides can co-exist
